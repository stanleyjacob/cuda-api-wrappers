...
//	silent_warning(tmpnam);
	auto ptx_filename = std::tmpnam(nullptr);
	std::ofstream ptx_file(ptx_filename);
	ptx_file << ptx_file_contents;
	return ptx_filename;
}
...
int main(int argc, char** argv)
{
    ...
    report_current_context("Before context creation");

    cuda::context::current::scoped_override_t context_setter { context };

    report_current_context("After context setter");
// first search for the module path before we load the results
    ...
    report_current_context("Before enqueue of dummy kernel_launch");
    ...
    report_current_context("Before enqueue of vector addition kernel_launch");
