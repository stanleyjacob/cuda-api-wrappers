...
public:
	...
	memory::region_t complete() const {
		...
		throw_if_error(status,
			"Failed completing the link with state at address " + cuda::detail::ptr_as_hex(handle_));
...
public: // friendship
	...
	friend link_t link::detail::wrap(context::handle_t context, link::handle_t handle, link::options_t, bool take_ownership) noexcept;
...
public: // constructors and destructor
	...
	~link_t()
	{
		if (owning) {
			...
			throw_if_error(status,
				std::string("Failed destroying the link " + detail::ptr_as_hex(handle_) +
...
inline link_t create(link::options_t options)
{
